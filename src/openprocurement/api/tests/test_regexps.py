...
class TestCoordinatesRegExp(unittest.TestCase):
    ...
    def test_coordinates_reg_exp(self):
        ...
        self.assertEqual(
            '1.1234567890', COORDINATES_REG_EXP.match('1.1234567890').group()
        )
        self.assertEqual('12', COORDINATES_REG_EXP.match('12').group())
        self.assertEqual(
            '12.1234567890', COORDINATES_REG_EXP.match('12.1234567890').group()
        )
        self.assertEqual('123', COORDINATES_REG_EXP.match('123').group())
        self.assertEqual(
            '123.1234567890',
            COORDINATES_REG_EXP.match('123.1234567890').group()
        )
        self.assertEqual('0', COORDINATES_REG_EXP.match('0').group())
        self.assertEqual(
            '0.1234567890', COORDINATES_REG_EXP.match('0.1234567890').group()
        )
        self.assertEqual(
            '-0.1234567890', COORDINATES_REG_EXP.match('-0.1234567890').group()
        )
        ...
        self.assertEqual(
            '-12.1234567890',
            COORDINATES_REG_EXP.match('-12.1234567890').group()
        )
        self.assertEqual('-123', COORDINATES_REG_EXP.match('-123').group())
        self.assertEqual(
            '-123.1234567890',
        ...
        self.assertNotEqual('1..1', COORDINATES_REG_EXP.match('1..1').group())
        self.assertNotEqual(
            '1.1.1', COORDINATES_REG_EXP.match('1.1.1').group()
        )
...
def suite():
    ...
    suite.addTest(unittest.makeSuite(TestCoordinatesRegExp))
    return suite
